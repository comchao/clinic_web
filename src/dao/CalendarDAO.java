...
public class CalendarDAO {
	...
	public ArrayList<CalBean> selectIdCalendar(int id_calendar) {
		...
		try {
			...
			preparedStmt.setInt(1, id_calendar);
			rs = preparedStmt.executeQuery();
			CalBean calbean;
			while (rs.next()) {
				calbean = new CalBean();
	...
	// เพิ่มข้อมูลนัดหมายล่าสุด insertAppointment
	public static CalendarBean insertAppointment(CalendarBean bean) {

		String insertSQL = "insert into calendar (id,id_calendar,title,date,date_time,id_ownerdata,note)"
				+ "values(?,?,?,?,?,?,?);";
		try {
			dbconConnection = ConnectionManager.getConnection();
			pre = dbconConnection.prepareStatement(insertSQL);
