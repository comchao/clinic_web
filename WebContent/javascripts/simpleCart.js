function cart(A) {
	...
	this.userEmail = A;
	this.ItemColumns = [ "Image", "Name", "Price", "Options", "Quantity",
			"Total" ];
	...
	this.add = function() {
		...
		for (B = 0; B < this.items.length; B++) {
			...
			if (tempItem.equalTo(newItem)) {
				tempItem.addValue("quantity", (parseInt(tempItem
						.getValue("quantity")) + parseInt(newItem
						.getValue("quantity"))));
				this.totalPrice = this.totalPrice
						+ parseFloat(tempItem.getValue("price"));
				isnew = false
			}
		}
		if (isnew) {
			...
			this.totalPrice = this.totalPrice
					+ parseFloat(newItem.getValue("price"))
		}
		this.updateCookie();
		this.updatePageElements();
		return
	...
	this.addItem = function(C) {
		...
		for (B = 0; B < this.items.length; B++) {
			var D = this.items[B];
			if (D.equalTo(C)) {
				D.addValue("quantity", parseInt(C.getValue("quantity"))
						+ parseInt(D.getValue("quantity")));
				this.totalItems = this.totalItems
						+ parseInt(C.getValue("quantity"));
				this.totalPrice = this.totalPrice
						+ parseInt(C.getValue("quantity"))
						* parseFloat(C.getValue("price"));
				return
			}
		}
		this.items[this.items.length] = C;
		this.totalItems = this.totalItems + parseInt(C.getValue("quantity"));
		this.totalPrice = this.totalPrice + parseInt(C.getValue("quantity"))
				* parseFloat(C.getValue("price"));
		return
	};
	...
	this.deleteItem = function(C) {
		...
		for (x = 0; x < this.items.length; x++) {
			...
			if (tempItem.equalTo(C)) {
				found = true;
				this.totalItems = this.totalItems
						- parseFloat(tempItem.getValue("quantity"));
				this.totalPrice = this.totalPrice
						- parseFloat(tempItem.getValue("price"))
			}
			if (found) {
				...
				for (C = 0; C < this.ItemColumns.length; C++) {
					...
					if (this.ItemColumns[C] == "Image") {
						if (tempItem.getValue("image")) {
							tempCell.innerHTML = '<img src="'
									+ tempItem.getValue("image") + '" />'
						}
					}
					if (this.ItemColumns[C] == "Name") {
						...
						if (this.ItemColumns[C] == "Price") {
							tempCell.innerHTML = this
									.returnFormattedPrice(tempItem
											.getValue("price"))
						} else {
							if (this.ItemColumns[C] == "Options"
									&& this.options()) {
								tempCell.innerHTML = tempItem.optionList()
							} else {
								if (this.ItemColumns[C] == "Quantity") {
									tempCell.innerHTML = '<input type="text" onblur="simpleCart.updateQuantity('
	...
	this.checkOut = function() {
		...
		var F, E = 0, G, C;
		var B = "https://www.paypal.com/cgi-bin/webscr?cmd=_cart&upload=1&business="
				+ this.userEmail + "&currency_code=USD&lc=US";
		C = 0;
		for (C = 0; C < this.items.length; C++) {
			tempItem = this.items[C];
			E = C + 1;
			B = B + "&item_name_" + E + "=" + tempItem.getValue("name")
					+ "&item_number_" + E + "=" + E + "&quantity_" + E + "="
					+ tempItem.getValue("quantity") + "&amount_" + E + "="
					+ this.returnFormattedPrice(tempItem.getValue("price"))
					+ "&no_shipping_" + E + "=0&no_note_" + E + "=1";
			if (tempItem.optionList()) {
				B = B + "&on0_" + E + "=Options&os0_" + E + "="
						+ tempItem.optionList()
			}
		}
...
function item() {
	...
	this.equalTo = function(A) {
		...
		var B = 0;
		for (B = 0; B < this.names.length; B++) {
			if (this.names[B] != "quantity"
					&& (A.getValue(this.names[B]) != this.values[B])) {
				return false
			}
		}
		return true
	...
	this.cookieString = function() {
		...
		for (A = 1; A < this.names.length; A++) {
			returnString = returnString + "," + this.names[A] + "="
					+ this.values[A]
		}
		return returnString
	};
	...
	this.optionList = function() {
		...
		var A = 0;
		for (A = 0; A < this.names.length; A++) {
			if (this.names[A] != "quantity" && this.names[A] != "price"
					&& this.names[A] != "name" && this.names[A] != "image") {
				returnString = returnString + this.names[A] + ":"
						+ this.values[A] + ", "
			}
		}
		while (returnString.charAt(returnString.length - 1) == ","
				...
				|| returnString.charAt(returnString.length) == ":") {
...
var getElementsByClassName = function(B, A, C) {
	if (document.getElementsByClassName) {
		getElementsByClassName = function(I, L, H) {
			H = H || document;
			var D = H.getElementsByClassName(I), K = (L) ? new RegExp("\\b" + L
					+ "\\b", "i") : null, E = [], G;
		...
		if (document.evaluate) {
			getElementsByClassName = function(M, P, L) {
				P = P || "*";
				L = L || document;
				var F = M.split(" "), N = "", J = "http://www.w3.org/1999/xhtml", O = (document.documentElement.namespaceURI === J) ? J
						: null, G = [], D, E;
			...
			getElementsByClassName = function(O, R, N) {
				R = R || "*";
				N = N || document;
				var H = O.split(" "), Q = [], D = (R === "*" && N.all) ? N.all
						: N.getElementsByTagName(R), M, J = [], L;
